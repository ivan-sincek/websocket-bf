...
function basic () {
	proceed=false
	echo "WebSocket BF v1.6 ( github.com/ivan-sincek/websocket-bf )"
	echo ""
	echo "--- Single request ---"
...
function get_sid () {
	...
	curl -s -H "Connection: close" -H "Accept-Encoding: gzip, deflate" -H "Authorization: Bearer ${2:-null}" "${1}/socket.io/?EIO=3&transport=polling" | gunzip -c | grep -Po '(\{(?:[^\{\}]+|(?-1))+\})' | jq -r '.sid'
}
...
if [[ $proceed == true ]]; then
	...
	if [[ ! -z ${args[wordlist]} ]]; then
		count=0
		for entry in $(cat "${args[wordlist]}"); do
			count=$((count + 1))
